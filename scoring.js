...
export function scoreCompany(record) {
  const weights = Object.fromEntries(COMPANY_DIMENSIONS.map(d => [d.key, d.defaultWeight]));
...
export function rankCompanies(records) {
  return records.map(scoreCompany).sort((a, b) => b.totalScore - a.totalScore);
}
