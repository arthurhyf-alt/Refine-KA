...
HOST = "127.0.0.1"
PORT = int(os.environ.get("REFINE_KA_PORT", "8775"))
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/health":
            payload = json.dumps({"status": "ok", "version": "4.3.0"}).encode("utf-8")
        ...
        if self.path == "/api/sources":
                ...
                try:
                    ...
                    rows = conn.execute(
                        """SELECT DISTINCT source_id, source_name, url, region, dimension, source_grade
                           FROM documents
                           WHERE source_id LIKE 'discovered-%' OR source_id LIKE 'semantic-%' OR source_id LIKE 'section-%'
            ...
            return
        if self.path == "/api/discovery/report":
            report_path = ROOT / "data" / "discovery-report.json"
            self.send_json(json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {"status": "never_run"})
            return
        if self.path == "/api/policy/report":
            report_path = ROOT / "data" / "policy-monitor-report.json"
            self.send_json(json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {"status": "never_run"})
            return
        if self.path == "/api/semantic/profiles":
            profile_path = ROOT / "config" / "semantic_profiles.json"
            self.send_json(json.loads(profile_path.read_text(encoding="utf-8")))
            return
        super().do_GET()
    ...
    def do_POST(self):
        if self.path == "/api/pipeline/run":
            ...
            return
        if self.path == "/api/semantic/feedback":
            try:
                length = int(self.headers.get("Content-Length", "0"))
                body = json.loads(self.rfile.read(length).decode("utf-8"))
                from semantic_discovery import record_feedback
                example = record_feedback(body["dimension"], body["verdict"], body.get("text", ""), body.get("url", ""), body.get("industryId", ""))
                self.send_json({"accepted": True, "example": example}, status=201)
            except Exception as error:
                self.send_json({"accepted": False, "message": str(error)}, status=400)
            return
        self.send_error(404)
