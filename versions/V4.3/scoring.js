...
export const COMPANY_DIMENSIONS = [
  ...
  { key: "market", label: "市场需求与行业地位", defaultWeight: 20 },
  { key: "expansion", label: "资本与扩张", defaultWeight: 20 },
  { key: "talent", label: "人才与组织活跃度", defaultWeight: 10 },
  { key: "innovation", label: "技术与产品突破", defaultWeight: 15 },
  { key: "policy", label: "政策与荣誉", defaultWeight: 15 },
];
...
export function scoreCompany(record, weights = companyDefaultWeights()) {
  ...
  if (!metrics.innovation) {
    const legacy = [metrics.technology, metrics.product].filter(item => item && item.score !== null && item.score !== undefined);
    if (legacy.length) metrics.innovation = {
      score: legacy.reduce((sum, item) => sum + Number(item.score), 0) / legacy.length,
      confidence: legacy.reduce((sum, item) => sum + Number(item.confidence || 0), 0) / legacy.length,
      evidence: legacy.map(item => item.evidence).filter(Boolean).join('；'),
      evidenceItems: legacy.flatMap(item => item.evidenceItems || []),
    };
  }
  const scored = scoreRecord({ ...record, metrics }, weights, COMPANY_DIMENSIONS);
  const domains = new Set(COMPANY_DIMENSIONS.flatMap(d => (metrics[d.key]?.evidenceItems || []).map(item => {
    try { return new URL(item.url).hostname.replace(/^www\./, ''); } catch { return ''; }
  })).filter(Boolean));
  const formal = scored.coverage >= 0.6 && domains.size >= 2;
  return { ...scored, independentSourceCount: domains.size, formal, tier: formal ? scored.tier : "数据不足" };
}
...
export function rankCompanies(records, weights = companyDefaultWeights()) {
  return records.map(record => scoreCompany(record, weights)).sort((a, b) => Number(b.formal) - Number(a.formal) || b.coverage - a.coverage || b.totalScore - a.totalScore);
}
