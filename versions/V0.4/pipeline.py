...
def aggregate(conn):
    ...
    for row in rows:
        ...
        entry = grouped.setdefault(key, {"id": f"{industry_id}-{region}", "industry": industry_name, "region": region, "updatedAt": observed_at[:10], "metrics": {}})
    ...
    for industry in load_json(CONFIG / "industries.json"):
    ...
    output = []
    dimensions = ["export", "industrial", "jobs", "finance", "technology", "demand", "policy"]
