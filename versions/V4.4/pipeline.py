...
from semantic_discovery import discover as semantic_discover, REPORT_PATH as DISCOVERY_REPORT_PATH
from policy_monitor import scan_sections as scan_policy_sections, REPORT_PATH as POLICY_REPORT_PATH
...
def aggregate(conn):
    ...
    rows = conn.execute(
        """SELECT o.industry_id, o.industry_name, o.region, o.dimension, o.score, o.confidence,
           ...
           WHERE julianday(o.observed_at) >= julianday('now','-180 days')
...
def run_pipeline(only_source=None):
    ...
    industries = load_json(CONFIG / "industries.json")
    ns_industries = load_json(CONFIG / "ns_industries.json")
    industry_by_code = {item.get("code"): item for item in industries}
    semantic_targets = []
    for ns in ns_industries:
        primary = next((industry_by_code.get(code) for code in ns.get("codes", []) if industry_by_code.get(code)), None)
        if primary:
            clean_name = re.sub(r"（.*?）|\(.*?\)", "", ns["name"]).strip()
            semantic_targets.append({**primary, "name": ns["name"], "keywords": list(dict.fromkeys([ns["name"], clean_name, *primary.get("keywords", [])]))})
    discovery_report = {"mode": "fixed_sources_only", "candidatesAccepted": 0}
    if not only_source:
        policy_sources, policy_report = scan_policy_sections()
        sources.extend(policy_sources)
        semantic_sources, discovery_report = semantic_discover(
            semantic_targets, discovery_config.get("endpoint", "https://news.google.com/rss/search?q={query}&hl=zh-CN&gl=CN&ceid=CN:zh-Hans")
        )
        sources.extend(semantic_sources)
        discovery_report["policyMonitor"] = policy_report
    ...
    # Keep historical evidence. Content hashes and observation uniqueness
    # prevent duplicates, while preserving an auditable time series across runs.
    for index, source in enumerate(sources):
        try:
            ...
            extraction_industries = industries
            if source.get("target_industry_id"):
                extraction_industries = [item for item in industries if item["id"] == source["target_industry_id"]]
            observations = extract_observations(text, source, extraction_industries)
    ...
    conn.commit()
    conn.close()
    return {"runId": run_id, "status": status, "sourceCount": len(sources), "successCount": success, "observationCount": observation_count, "industryRows": len(output), "discovery": discovery_report, "errors": errors}
...
def latest_status():
    ...
    if DISCOVERY_REPORT_PATH.exists():
        try:
            result["discovery"] = load_json(DISCOVERY_REPORT_PATH)
        except (OSError, json.JSONDecodeError):
            pass
    if POLICY_REPORT_PATH.exists():
        try:
            result.setdefault("discovery", {})["policyMonitor"] = load_json(POLICY_REPORT_PATH)
        except (OSError, json.JSONDecodeError):
            pass
    return result
